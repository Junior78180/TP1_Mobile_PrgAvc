...
def calculate_speedup(data):
    ...
    p_min = np.min(nombre_process)
    T1 = np.median(temps_execution[nombre_process == p_min])
...
def plot_speedup(speedup_data, nombre_process_data, ntot_values):
    ...
    plt.xlabel('Nombre de processus par worker')
    plt.ylabel('Speedup')
...
if __name__ == "__main__":
    file_path = 'out_mws_salleG26_4c.txt'  # Path to the data file
